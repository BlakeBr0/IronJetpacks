                ...
                if (!mc.isPaused()) {
                    ...
                    if (!chest.isEmpty() && item instanceof JetpackItem && JetpackUtils.isFlying(mc.player)) {
                        if (ModConfigs.ENABLE_JETPACK_PARTICLES.get() && (mc.options.particles != ParticleStatus.MINIMAL)) {
                            ...
                            Pos3d vLeft = new Pos3d(-0.18, -0.90 + sneakBonus[1], -0.30 + sneakBonus[0]).xRot(0).rotateYaw(mc.player.yBodyRot);
                            Pos3d vRight = new Pos3d(0.18, -0.90 + sneakBonus[1], -0.30 + sneakBonus[0]).xRot(0).rotateYaw(mc.player.yBodyRot);
